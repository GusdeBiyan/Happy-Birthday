...
// trigger to play music in the background with sweetalert
 function playAudioAsync(selector) {
    ...
    // Async function to handle the animation and background change
     async function handleAnimationAndBackgroundChange() {
        // document.getElementById('lightSwitch').style.display = 'block';

        // Listen for the turn on button click
        const turnOnButton = document.getElementById('turnOnButton');
        turnOnButton.addEventListener('click', async () => {
            await playAudioAsync('.song2');
            setTimeout(async () => {
        await changeBackgroundColor('#FFFF');
        playAudioAsync('.song');
        animationTimeline();
    ...
    // Execute everything on window load
    window.addEventListener('load', handleAnimationAndBackgroundChange);

 async function changeBackgroundColor(color) {
     const body = document.body;
    body.classList.add('background-transition');
    
    // Simulate a delay before changing the background color
    setTimeout(() => {
